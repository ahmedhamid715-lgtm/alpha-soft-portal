...
import { importEcommerceProductCsvAction } from "../../../actions";
...
import type { EcommerceCatalogImportResult } from "@/server/services/ecommerce-catalog-service";
import type { EcommerceImportBatch } from "@/generated/prisma/client";

/**
 * Manual CSV catalog import (Build 33 — Roadmap Module 27). One row
 * creates one PRODUCT — a row that also carries both `sku` and `price`
 * additionally creates one "Default" variant carrying that data (see
 * `src/lib/ecommerce/csv-import.ts`'s own doc comment). Every import
 * batch is append-only evidence — it can never be edited or deleted
 * here. Required columns: title; optional: handle, productType, vendor,
 * requiredForLaunch (true/false), sku, price.
 */
export function ImportTab({ storeId, initialBatches, canManage }: { storeId: string; initialBatches: EcommerceImportBatch[]; canManage: boolean }) {
  const [pending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<EcommerceCatalogImportResult | null>(null);
  ...
  function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      ...
      const importResult = await importEcommerceProductCsvAction({ storeId, fileContent });
        ...
        <Card>
          <CardContent>
            <form action={handleSubmit} className="flex flex-col gap-3">
              <p className="text-xs text-muted-foreground">CSV columns: title, handle, productType, vendor, requiredForLaunch (true/false), sku, price. A row with both sku and price also creates one default variant.</p>
