...
import { recordLocalSeoAuditRunAction } from "../../../actions";
...
import type { LocalSeoAuditRun } from "@/generated/prisma/client";

const ISSUE_TYPES = ["NAP_INCONSISTENCY", "MISSING_LISTING", "PROFILE_INCOMPLETE", "UNVERIFIED_PROFILE", "REVIEW_RESPONSE_BACKLOG", "OTHER"];

export function AuditsTab({ locationId, initialRuns, canManageMeasurements }: { locationId: string; initialRuns: LocalSeoAuditRun[]; canManageMeasurements: boolean }) {
  ...
  function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      const result = await recordLocalSeoAuditRunAction({
        locationId,
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {initialRuns.length === 0 ? (
        <EmptyState icon={ClipboardList} title="No audits recorded yet" description="Record a Local SEO audit to start tracking issues over time." />
      ) : (
