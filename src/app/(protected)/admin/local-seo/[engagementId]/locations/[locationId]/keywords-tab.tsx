...
import { createLocalSeoKeywordAction, archiveLocalSeoKeywordAction, reactivateLocalSeoKeywordAction, recordLocalRankObservationAction } from "../../../actions";
...
import type { LocalSeoKeywordListItem } from "@/server/services/local-seo-engagement-service";
...
/** Local keyword tracking is Local Pack search-surface only (`searchSurface` fixed) — never mixed with SEO OS's own organic rank tracking. */
export function KeywordsTab({ locationId, initialItems, canManage, canManageMeasurements }: { locationId: string; initialItems: LocalSeoKeywordListItem[]; canManage: boolean; canManageMeasurements: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {initialItems.length === 0 ? (
        <EmptyState icon={Tags} title="No local keywords tracked yet" description="Add a keyword to start recording its Local Pack ranking." />
      ) : (
        <div className="overflow-x-auto rounded-lg border border-border" tabIndex={0} role="region" aria-label="Local keywords table, scrollable on narrow viewports">
                  ...
                  <tr className="border-b border-border last:border-0 hover:bg-muted/30">
                    ...
                    <td className="px-4 py-2.5 text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        {canManage ? (
                          k.status === "ACTIVE" ? (
                            <Button size="sm" variant="ghost" disabled={pending} onClick={() => runAction(() => archiveLocalSeoKeywordAction({ keywordId: k.id }))}>
                              Archive
                            </Button>
                          ) : (
                            <Button size="sm" variant="ghost" disabled={pending} onClick={() => runAction(() => reactivateLocalSeoKeywordAction({ keywordId: k.id }))}>
                              Reactivate
                            </Button>
                          )
                        ) : null}
...
function RecordObservationForm({ keywordId, onDone, onError }: { keywordId: string; onDone: () => void; onError: (e: string | null) => void }) {
  ...
  function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      const result = await recordLocalRankObservationAction({
        keywordId,
